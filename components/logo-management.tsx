...
import { partnerLogos, type PartnerLogo, type CountryLogos } from "@/data/partner-logos"
...
export default function LogoManagement() {
  ...
  const categories = [
    { value: "producer", label: "Producer", color: "bg-green-100 text-green-800" },
    { value: "technology", label: "Technology", color: "bg-blue-100 text-blue-800" },
    { value: "investor", label: "Investor", color: "bg-purple-100 text-purple-800" },
  ...
  const handleSave = () => {
    ...
    if (editingLogo) {
      const index = updatedLogos[selectedCountry].findIndex((logo) => logo.id === editingLogo.id)
      if (index !== -1) {
  ...
  const handleDelete = (logoId: string) => {
    const updatedLogos = { ...logoData }
    updatedLogos[selectedCountry] = updatedLogos[selectedCountry].filter((logo) => logo.id !== logoId)
    setLogoData(updatedLogos)
  }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Manage partner logos displayed in the carousel for each country</p>
      </div>
                    ...
                    <Card key={logo.id} className="hover:shadow-md transition-shadow">
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-3">
                          <Badge className={getCategoryStyle(logo.category)}>{logo.category}</Badge>
