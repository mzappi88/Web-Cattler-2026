  ...
  useEffect(() => {
    ...
    const detectCountry = async () => {
      ...
      try {
        ...
        console.log("🌍 Full detection summary:", {
          userCountryCode,
          finalCountry,
          browserLanguage: typeof window !== 'undefined' ? navigator.language : 'N/A',
          timezone: typeof window !== 'undefined' ? Intl.DateTimeFormat().resolvedOptions().timeZone : 'N/A',
          supportedCountries: ["US", "CA", "AR", "PY", "UY", "BO", "BR", "MX", "CH"],
          isSupported: ["US", "CA", "AR", "PY", "UY", "BO", "BR", "MX", "CH"].includes(userCountryCode),
          isSpanishSpeaking: spanishSpeakingCountries.includes(userCountryCode),
          isEnglishSpeaking: englishSpeakingCountries.includes(userCountryCode)
        });
        ...
        // Additional check: if browser language is Spanish, prefer AR over OT$ES
        let finalFallbackCountry = fallbackCountry;
        if (typeof window !== 'undefined') {
          const browserLanguage = navigator.language.toLowerCase();
          const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
          
          console.log("🌍 Fallback check - Browser language:", browserLanguage, "Timezone:", timezone);
          
          // If browser language is Spanish and timezone suggests Argentina, use AR
          if (browserLanguage.startsWith("es") && 
              (timezone.includes("America/Argentina") || 
               timezone.includes("America/Buenos_Aires") ||
               timezone.includes("America/Cordoba") ||
               timezone.includes("America/Rosario"))) {
            console.log("🌍 Fallback: Spanish browser + Argentina timezone, using AR");
            finalFallbackCountry = "AR";
          }
        }
        
        console.log("🌍 Final fallback country:", finalFallbackCountry);
        
        // Cache the fallback result
        localStorage.setItem("cattler-country", finalFallbackCountry);
        localStorage.setItem("cattler-country-last-detection", now.toString());
        
        setDetectedCountry(finalFallbackCountry);
        hasDetected.current = true;
      } finally {
        setIsDetecting(false);
      }
