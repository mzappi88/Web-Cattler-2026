...
export default function PaymentPageRoute() {
  ...
  const handleBack = () => {
    ...
    if (paymentData?.plan?.id && paymentData?.billingCycle) {
      router.push(`/pricing/checkout/${paymentData.plan.id}?billing=${paymentData.billingCycle}`)
    } else {
      router.push("/pricing")
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-cattler-light-teal/10 to-cattler-teal/20 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-cattler-navy font-lato">Carregando dados de pagamento...</p>
        </div>
      </div>
    )
  }
  ...
  return <PaymentPage paymentData={paymentData} onBack={handleBack} onSuccess={handleSuccess} />
}
